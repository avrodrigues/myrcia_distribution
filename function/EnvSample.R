...
#' 
#' Ecological Niche Model's predictions could be improved by using unbiased data
#' sets for calibrating the algorithms. 
#' With this function users could filter their raw species occurrences to avoid
#' environmental biases. The user should select which variables (filters)
#' are adequate for filtering the raw species occurrences:
#' e.g. selecting the variables that are important for the species, or, in case
#' there is no information about the importance of the variables, selecting 
...
#' @param filters a list of vectors with the values of the variables for the 
#' species occurrences (users can get these vectors by using the function 
#' extract from the raster R-package) 
#' @param res a list of numbers that set the size of the grid to filter the 
#' data for each variable
#' @param do.plot TRUE/FALSE. If FALSE, the function does not return a plot
#' showing the raw and filtered data sets.
#' 
#' @examples \dontrun{
...
envSample<- function (coord, filters, res, do.plot=TRUE){
  ...
  selection_NA<- sqldf(paste ("select real_p.lon, real_p.lat, pot_pp.groupID",   
                              sql1, "from pot_pp left join real_p on", 
                              conditions, sep=" "))
